...
def fix(conn):
  ...
  selectQuery = Template("SELECT L.id, L.body FROM $tableName AS L LEFT JOIN body AS R ON L.id=R.news_id WHERE L.source_name='BBC' AND L.actual_source_name='APP' AND R.news_id IS NULL"). substitute(tableName=tableName)
  print(selectQuery)
